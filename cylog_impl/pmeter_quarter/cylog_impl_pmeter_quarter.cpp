...
CL_TYPE_t CYLogPMeter15Impl::traverse(log_read_cb_t cb=nullptr) {
...
CYLogPMeter15Impl::CYLogPMeter15Impl(const std::string & dir, std::shared_ptr<StoreAbs> &store, std::unique_ptr<CLFile::FileDesc> pFDesc ):
...
CYLogImplAbs* CyLogPMeter15Factory::create(std::shared_ptr<StoreAbs> &store, std::string logDir, uint32_t  fileSize, uint8_t fileCount, std::string prefix) {
    ...
    return new CYLogPMeter15Impl(logDir, store, std::move(pFDesc) );
}
